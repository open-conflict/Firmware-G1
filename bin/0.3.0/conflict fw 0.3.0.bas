...
Dim Analog_status As Byte
Dim Analog_min As Byte
Dim Analog_anlauf As Byte
Dim Analog_anlauf_t As Byte
Dim Analog_anlauf_schwelle As Byte                          'Bei wie viel % Kühlbedarf der Lüfter wieder an geht
Analog_auto Alias F_aus_auto.6                              '1 wenn automatische Regelung
Analog_aus Alias F_aus_auto.7                               '1 wenn Lüfter aus gehen darf
Dim Analog_man As Byte                                      'Manueller Wert
Dim Min_a_t(24) As Byte                                     '24 Minimum Temperaturen
Dim Max_a_t(24) As Byte                                     '24 Maximum Temperaturen
Dim Kuehlung_analog As Byte

Dim Analog_spannung As Byte

Dim F_aus_auto As Byte                                      'Hier wird aus und auto modus für alle 3 Fan Kanäle und Analog gespeichert
'                        Bitweise
 ' 7               6         5       4         3      2        1       0
 ' Analog_aus  Analog_auto  F1_aus  F1_auto  F2_aus  F2_auto  F3_aus  F3_auto
...
$eepromhex
 ...
 'Analog:
 Data 100 , 20 , 20 , 120                                   'Min, Anlaufzeit, % Schwelle, Manuell
...
Dim Analog_min_e As Eram Byte                               'Minimaler PWM Wert
Dim Analog_anlauf_e As Eram Byte                            'Konstante für Anlaufzeit
Dim Analog_anlauf_schwelle_e As Eram Byte                   'Bei wie viel % Kühlbedarf der Lüfter wieder an geht
Dim Analog_man_e As Eram Byte                               'Manueller Wert
Dim Min_a_t_e(24) As Eram Byte                              '24 Minimum Temperaturen
Dim Max_a_t_e(24) As Eram Byte

Analog_status = 1                                           'Lüfter Status
Analog_min = Analog_min_e                                   'Minimaler PWM Wert
Analog_anlauf = Analog_anlauf_e                             'Konstante für Anlaufzeit
Analog_anlauf_t = Analog_anlauf
Analog_anlauf_schwelle = Analog_anlauf_schwelle_e           'Bei wie viel % Kühlbedarf der Lüfter wieder an geht
Analog_man = Analog_man_e                                   'Manueller Wert
For Ctr = 1 To 24
    Min_a_t(ctr) = Min_a_t_e(ctr)                           '24 Minimum Temperaturen
    Max_a_t(ctr) = Max_a_t_e(ctr)                           '24 Maximum Temperaturen
Next
...
'###############################################################################
  ...
  If Analog_auto = 1 Then                                   'Wenn Analog automatisch geregelt werden soll

      Kuehlung_analog = 0

      For Ctr = 1 To 24                                     'berechne für jede Temperatur den Kühlungsbedarf

         If Temperaturen(ctr) >= Min_a_t(ctr) And Temperaturen(ctr) <= Max_a_t(ctr) Then       'Temperatur ist zwischen Min und Max

            Temp_byte = Max_a_t(ctr) - Min_a_t(ctr)
            Temp_byte = 100 / Temp_byte

            Temp_byte2 = Temperaturen(ctr) - Min_a_t(ctr)
            Temp_byte2 = Temp_byte2 * Temp_byte

            If Temp_byte2 > Kuehlung_analog Then Kuehlung_analog = Temp_byte2       'Speichere den höchsten Kühlungsbedarf
         ...
         If Temperaturen(ctr) > Max_a_t(ctr) Then           'Temperatur ist Größer als Max
      ...
      Select Case Analog_status

         Case 0:                                            'Analog ist aus


            Analog = 0                                      'Analog aus
            Analog_spannung = 0
            If Analog_aus = 1 Then                          'Analog darf aus
               If Kuehlung_analog > Analog_anlauf_schwelle Then
                  Incr Analog_status                        'Analog soll anlaufen
                  Analog_anlauf_t = Analog_anlauf           'Anlaufzeit zurücksetzen
               End If
            Else                                            'Analog sollte aber laufen
               Incr Analog_status                           'Analog soll anlaufen
               Analog_anlauf_t = Analog_anlauf              'Anlaufzeit zurücksetzen
            End If

         Case 1:                                            'Analog läuft an
         ...
         Case 2:                                            'Lüfter läuft Normal
            ...
            If Analog_aus = 1 And Kuehlung_analog = 0 Then Analog_status = 0       'Wenn keine Kühlung benötigt und der Lüfter aus gehen darf
            Temp_byte2 = 120 - Analog_min
            Temp_single = Temp_byte2 / 100
            Temp_byte2 = Temp_single * Kuehlung_analog
            Temp_single = Temp_byte2 + Analog_min
            Analog_spannung = Temp_single
  ...
  Else                                                      'Wenn Manuelle Einstellung
      ...
      Select Case Analog_status
         ...
         Case 0:                                            'Analog ist aus
            ...
            Analog = 0
            Analog_spannung = 0
            If Analog_man > 0 Then
               Incr Analog_status
               Analog_anlauf_t = Analog_anlauf              'Analog gibt x s lang gas
            End If
         ...
         Case 2 :                                           'Analog Läuft Normal

            If Analog_man = 0 Then                          'Wenn 0 eingestellt, soll die Analog auch in den Auszustand
               Analog_status = 0                            'Analog soll aus
            Else
               Analog_spannung = Analog_man
               Temp_single = Analog_man
               Temp_single = Temp_single * 1.7
               Temp_byte = Temp_single
               Analog = Temp_byte
            End If
      ...
      If Temp_byte = Val(datenwort(2)) Then                 'Wenn die angegebene Datenwortanzahl auch mit der empfangenen überein stimmt
         ...
         Select Case Datenwort(1)                           'Führe den jeweiligen Befehl aus
            ...
            Case "INST":                                    'Das GUI verlangt nach der Initialisierung
               ...
               Print #1 , Analog_man ; "#" ; Analog_anlauf ; "#" ; Analog_min ; "#" ; Analog_auto ; "#" ; Analog_aus ; "#";
               ...
               Print #1 , Hex(adr_t(64)) ; "#0#3#0#" ;      'Firmware Version!

               Print #1 , Analog_anlauf_schwelle ; "#" ; F1_anlauf_schwelle ; "#" ; F2_anlauf_schwelle ; "#" ;
               Print #1 , F3_anlauf_schwelle ; "#"
            ...
            Case "MIAA":                                    'Minmax Analog Kanal

               For Ctr = 1 To 24
                  Temp_byte2 = Ctr + 2
                  Min_a_t(ctr) = Val(datenwort(temp_byte2))
                  Temp_byte = Min_a_t_e(ctr)
                  If Min_a_t(ctr) <> Temp_byte Then         'Speichere falls geändert
                     Min_a_t_e(ctr) = Min_a_t(ctr)
                  End If
                  Temp_byte2 = Ctr + 26
                  Max_a_t(ctr) = Val(datenwort(temp_byte2))
                  Temp_byte = Max_a_t_e(ctr)
                  If Max_a_t(ctr) <> Temp_byte Then         'Speichere falls geändert
                     Max_a_t_e(ctr) = Max_a_t(ctr)
                  End If
               Next
            ...
            Case "KANA":                                    'Analog Kanal

               Analog_man = Val(datenwort(3))
               Temp_byte = Analog_man_e
               If Analog_man <> Temp_byte Then              'Speichere falls geändert
                  Analog_man_e = Analog_man
               End If

               Analog_anlauf = Val(datenwort(4))
               Temp_byte = Analog_anlauf_e
               If Analog_anlauf <> Temp_byte Then           'Speichere falls geändert
                  Analog_anlauf_e = Analog_anlauf
               End If

               Analog_min = Val(datenwort(5))
               Temp_byte = Analog_min_e
               If Analog_min <> Temp_byte Then              'Speichere falls geändert
                  Analog_min_e = Analog_min
               End If
               ...
               End If
               Analog_anlauf_schwelle = Val(datenwort(8))
               Temp_byte = Analog_anlauf_schwelle_e
               If Analog_anlauf_schwelle <> Temp_byte Then  'Speichere falls geändert
                  Analog_anlauf_schwelle_e = Analog_anlauf_schwelle
               End If


            Case "KAN1":                                    'Lüfter Kanal 1
...
'###############################################################################
'############ Konvertiere Wert sicher und rechtsbündig nach String #############
'###############################################################################
 ...
 Display_akt:
     ...
     Select Case Screen
            ...
            Case 1:
                 ...
                 Locate 1 , 1 : Lcd " Analog:  "            'Analognspannung
                 If Analog_spannung < 100 Then Lcd " "
                 Temp_byte = Analog_spannung / 10
                 Lcd Temp_byte ; ","
                 Temp_byte = Analog_spannung Mod 10
                 Lcd Temp_byte ; " V "
...
Count_startup_time:
     ...
     Select Case Analog_anlauf_t

         Case Is > 1 :
            Decr Analog_anlauf_t
...
If Gueltigkeit.0 = 1 Then                                   'DFM Wert ungültig
             ...
             ' X X X F1 F2 F3 T D
             If Analog_status = 2 Then                      'Analog läuft normal
                  Set Alarm_status.0
             Else                                           'Analog darf stehen
         ...
         Else                                               'DFM Wert gültig
             ...
             If Durchfluss >= Durchfluss_alarm Then
                 Reset Alarm_status.0
             Else
                 If Analog_status = 2 Then Set Alarm_status.0
             End If
...
Send_refresh:
        ...
        If Gui_version > 0 Then                             'Init wurde durchgeführt
            ...
            Print #1 , Temp_byte ; "#" ;
            Temp_byte = Fan3
            Print #1 , Temp_byte ; "#" ; Analog_spannung ; "#"
